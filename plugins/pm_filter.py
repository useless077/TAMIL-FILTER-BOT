...
from database.ia_filterdb import col, sec_col, db as skdb, sec_db, get_file_details, get_search_results, get_bad_files
...
from Tamilini.util.file_properties import get_name, get_hash, get_media_file_size
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ...
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ᴍᴏᴠɪᴇs ꜰʀᴏᴍ ʜᴇʀᴇ. ʀᴇǫᴜᴇsᴛ ɪᴛ ɪɴ ᴏᴜʀ <a href=https://t.me/MovieDiscussion24x7>🎥ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ᴏʀ ᴄʟɪᴄᴋ ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ 👇</b>\n\n<b>நீங்கள் இங்கிருந்து திரைப்படங்களைப் பெற முடியாது. எங்கள் <a href=https://t.me/MovieDiscussion24x7>🎥ᴍᴏᴠɪᴇ ɢʀᴏᴜᴘ</a> ல் கோருங்கள்.அல்லது கீழே உள்ள கோரிக்கை பொத்தானை கிளிக் செய்யவும்👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/MovieDiscussion24x7")]]))
    ...
    try:
        if int(query.from_user.id) not in [query.message.reply_to_message.from_user.id, 0]:
            return await query.answer(
                f"⚠️ ʜᴇʟʟᴏ{query.from_user.first_name},\nᴛʜɪꜱ ɪꜱ ɴᴏᴛ ʏᴏᴜʀ ᴍᴏᴠɪᴇ ʀᴇQᴜᴇꜱᴛ,\nʀᴇQᴜᴇꜱᴛ ʏᴏᴜʀ'ꜱ...\n\n⚠️ வணக்கம்,\n இது உங்கள் கோரிக்கை அல்ல,\n உங்கள் கோரிக்கையை கோருங்கள்...",
                show_alert=True,
            )
    except:
        pass
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and len(links) >= 1:
            await query.answer("Jᴏɪɴ ᴏᴜʀ Bᴀᴄᴋ-ᴜᴘ ᴄʜᴀɴɴᴇʟs ᴍᴀʜɴ! 😒 முதலில் சேனலில் சேருங்கள் தம்பி!😒", show_alert=True)
            return
     ...
     #   if not search:
      ...
      #      return
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/tamil5k")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            return
    # don't change anything without contacting me 
        ...
        buttons = [[
             ...
             InlineKeyboardButton('🤩 ᴍᴜʟᴛɪ-ᴅʟ', callback_data='download')
         ], [ 
        ...
        buttons = [[
            ...
            InlineKeyboardButton("👮🏻‍♂️ Cᴏɴᴛᴀᴄᴛ", url="telegram.me/TamilSupport")
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await client.edit_message_media(
            query.message.chat.id, 
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b><i>Tamilini Ai Try To Find Your Movie With Your Wrong Spelling.</i></b>")
        movienamelist = []
