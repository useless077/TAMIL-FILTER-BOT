...
from info import LOG_CHANNEL, REQST_CHANNEL as DUMP_GROUP
headers = {
...
async def link_handler(Client, message):
    ...
    try:
        ...
        if url.endswith("="):
           dump_file=await message.reply_video(url[:-1],caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies5K")
        else:
            dump_file=await message.reply_video(url,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies5K")
        ...
        try:
            if "/reel/" in url:
               ...
               try:
                   if ddinsta:
                      dump_file=await message.reply_video(content_value,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies5K")
                   else:
                       dump_file=await message.reply_video(content_value, caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies5K")
               except:
                   ...
                   dump_file=await message.reply_video(downfile,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilBots") 
            elif "/p/" in url:
                  meta_tag = requests.post("https://saveig.app/api/ajaxSearch", data={"q": link, "t": "media", "lang": "en"}, headers=headers)
                  if meta_tag.ok:
                     res=meta_tag.json()
              ...
              #    await message.reply(meta)
                  for i in range(len(meta) - 1):
                     ...
                     try:
                        dump_file=await message.reply_video(com.text,caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies5K")
                        await com.delete()
                     except:
                         pass 
            elif "stories" in url:
                  ...
                  if meta_tag.ok:
                     ...
                     meta=re.findall(r'href="(https?://[^"]+)"', res['data']) 
                      ...
                      return await message.reply("Oops something went wrong")
                  try:
                     dump_file=await message.reply_video(meta[0], caption="ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ʙʏ @TamilMovies5K")
                  except:
          ...
          #     await message.reply(tracemsg)
            ...
            if 'downfile' in locals():
                os.remove(downfile)
            await message.reply("<a href='https://t.me/TamilBots'>ᴜsᴇ ɴᴇᴡ ғᴇᴀᴛᴜʀᴇs</a>")
